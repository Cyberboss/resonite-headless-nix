...
let
  ...
  init-script = pkgs.writeShellScriptBin init-script-name ''
        ...
        exec ${lib.getExe pkgs.dotnetCorePackages.dotnet_10.runtime} ${headless-directory}/Resonite.dll -HeadlessConfig ${runtime-config-path} ${
          (
            if cfg.enable-rml then "-LoadAssembly ${headless-directory}/Libraries/ResoniteModLoader.dll" else ""
          )
        }
  '';

  config-json = jsonFormat.generate config-filename (
    cfg.config-json
    // {
