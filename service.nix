...
let
  ...
  rml = pkgs.fetchzip {
    url =
      "https://github.com/resonite-modding-group/ResoniteModLoader/releases/download/5.0.1/ResoniteModLoader.zip";
  ...
  depotdownloader = pkgs.depotdownloader.overrideAttrs (oldAttrs: {
    patches = (oldAttrs.patches or [ ]) ++ [
      (pkgs.fetchpatch {
        ...
        url =
          "https://github.com/Cyberboss/DepotDownloader/commit/0b0a47a3ace04e772ee0861c2228686fe26716bc.patch";
    ...
    if cfg.enable-rml then
      (lib.concatStringsSep "\n" [''
        if [ ! -f "${mod-builds-cache-directory}/ResoniteModLoader.dll" || ! -f "${mod-builds-cache-directory}/0Harmony.dll" ]; then
          ...
          cp -r ${config.rml-source.src} $PUBLISH_DIR/
          pushd $PUBLISH_DIR
  ...
  patchelf-command = ''
    ${
      lib.getExe pkgs.patchelf
    } --set-rpath "${pkgs.libpng}/lib:${pkgs.zlib}/lib:${pkgs.bzip2.out}/lib"'';
