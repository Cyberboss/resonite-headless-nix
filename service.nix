  ...
  init-script = pkgs.writeShellScriptBin init-script-name ''
    ...
    for dir in ${headless-directory}/runtimes/*/; do
      ...
      for file in ''${dir}native/*.so; do
        ...
        echo "Patching $file"
        ${pkgs.patchelf}/bin/patchelf --set-rpath "${pkgs.libpng}/lib:${pkgs.zlib}/lib:${pkgs.lbzip2}/lib" $file
      done
    done

    ${(if cfg.enable-rml then "${pkgs.systemd}/bin/systemd-notify --status=\"Installing ResoniteModLoader...\"" else "")}
    ${(if cfg.enable-rml then "cp -rf ${rml}/* ${headless-directory}/ && mkdir -p ${headless-directory}/rml_mods && chmod 770 ${headless-directory}/rml_mods && chmod 770 ${headless-directory}/Libraries && chmod -R 770 ${headless-directory}/Libraries/" else "")}
